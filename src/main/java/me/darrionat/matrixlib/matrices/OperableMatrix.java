...
public abstract class OperableMatrix implements Iterable<Quantity> {
     ...
     */
    public OperableMatrix(int rowAmount, int columnAmount, Quantity[][] entries) {
        ...
        if (entries == null) {
            this.entries = new Quantity[rowAmount][columnAmount];
     ...
     */
    public Quantity getValue(int row, int column) {
     ...
     */
    public Quantity[] getRow(int row) {
        return entries[row];
    }
     ...
     */
    public Quantity[] getColumn(int column) {
        Quantity[] toReturn = new Quantity[rowAmount];
    ...
    public void swapRows(int a, int b) {
        if (a == b) return;
        Quantity[] row1 = getRow(a);
        Quantity[] row2 = getRow(b);
        setRow(a, row2);
        setRow(b, row1);
    }
     ...
     */
    public void rowSum(int row, int addendRow, Quantity addendScalar) {
        for (int column = 0; column < columnAmount; column++) {
            Quantity toAdd = getValue(addendRow, column).multiply(addendScalar);
